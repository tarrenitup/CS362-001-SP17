	...
	**/
	public LinkedList<CalDay> getApptRange(LinkedList<Appt> appts,GregorianCalendar firstDay, GregorianCalendar lastDay){
			...
			if (!firstDay.before(lastDay)){
				throw new IllegalArgumentException ("Second date specified is not " + "before the first date specified.");
			}
			...
			for (int i = 0; i < appts.size(); i++){
				...
				LinkedList<GregorianCalendar> apptOccursOnDays = getApptOccurences(appt, firstDay, lastDay);
